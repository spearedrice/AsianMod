...
public class SlipperItem extends Item {
	...
	public InteractionResult use(Level level, Player player, InteractionHand hand) {
		...
		level.playSound(null, player.getX(), player.getY(), player.getZ(),
				CustomSounds.SLIPPER_THROWN, SoundSource.NEUTRAL,
				0.5F, 0.4F / (level.getRandom().nextFloat() * 0.4F + 0.8F));

		if (level instanceof ServerLevel serverLevel) {
			SlipperEntity slipper = new SlipperEntity(serverLevel, player);
			slipper.shootFromRotation(player, player.getXRot(), player.getYRot(), 0.0F, 1.5F, 1.0F);
			serverLevel.addFreshEntity(slipper);
		}
